...
function readChallengeEnvelope(filePath) {
  const absolutePath = path.resolve(filePath);
  const raw = parseJson(fs.readFileSync(absolutePath, "utf8"), absolutePath);
  const cachedProof =
    typeof raw?.cachedProof === "string"
      ? raw.cachedProof
      : typeof raw?.cached_proof === "string"
        ? raw.cached_proof
        : "";
  return {
    cachedProof,
    cachedChallenge: normalizeChallenge(raw),
  };
}
...
function createHeadersLike(values) {
...
function createBrowserContext(options) {
  ...
  const notifyResourceTiming = (entry) => {
    resourceEntries.push(entry);
    if (resourceEntries.length > 200) resourceEntries.shift();
    for (const observer of [...performanceObservers]) {
      if (!observer._active || !observer._types.has("resource")) continue;
      observer._queue.push(entry);
      managedSetTimeout(() => observer._flush(), 0);
    }
  };
  class PerformanceObserver {
    ...
    observe(options = {}) {
      const type = options.type || options.entryTypes;
      const types = Array.isArray(type) ? type : [type].filter(Boolean);
      for (const item of types) this._types.add(String(item));
      this._active = true;
      ...
      if (options.buffered) {
        ...
        managedSetTimeout(() => this._flush(), 0);
      }
    }
    disconnect() {
      this._active = false;
    ...
    _flush() {
      ...
      this._callback({ getEntries: () => records }, this);
    }
  }
  ...
  const browserPerformance = {
    ...
    getEntriesByName(name) {
      return resourceEntries.filter((entry) => entry.name === String(name));
    },
    getEntriesByType(type) {
      return String(type) === "resource" ? resourceEntries.slice() : [];
    },
  };
      ...
      const iframe = {
        ...
        contentWindow: {
          postMessage(message, origin) {
            if (process.env.SENTINEL_RUNNER_TRACE === "1") {
              process.stderr.write(`iframe postMessage type=${message?.type} flow=${message?.flow} origin=${origin}\n`);
            }
            Promise.resolve()
              .then(async () => {
  ...
  const wrappedFetch = async (input, init = {}) => {
    ...
    if (process.env.SENTINEL_RUNNER_TRACE === "1") {
      process.stderr.write(`fetch ${method} ${url} ping=${isSentinelPing}\n`);
    }
    if (isSentinelPing) {
      const responseStart = browserPerformance.now() + 12;
      const entry = {
        name: url,
        entryType: "resource",
        initiatorType: "fetch",
        startTime: start,
        requestStart: start + 3,
        responseStart,
        responseEnd: responseStart + 2,
        duration: responseStart + 2 - start,
        nextHopProtocol: "h2",
        transferSize: 128,
        encodedBodySize: 0,
        decodedBodySize: 0,
      };
      notifyResourceTiming(entry);
      return {
        ok: true,
        status: 204,
        ...
        headers: createHeadersLike({
          "s-cf-edge-msec": "8",
          "s-cf-origin-ttfb-msec": "12",
          "s-cf-tcp-rtt-msec": "34",
          "s-cf-quic-rtt-msec": "0",
        }),
        async text() {
          return "";
        },
    ...
    notifyResourceTiming({
      name: url,
      entryType: "resource",
      initiatorType: "fetch",
      startTime: start,
      requestStart: start + 2,
      responseStart: browserPerformance.now(),
      responseEnd: browserPerformance.now(),
      duration: Math.max(0, browserPerformance.now() - start),
      nextHopProtocol: "h2",
    });
    return response;
  };
  ...
  const window = Object.assign(windowTarget, {
    ...
    outerHeight: options.screen.height,
    devicePixelRatio: 1,
    chrome: { runtime: {} },
    caches: {},
    indexedDB: {},
...
async function main(argv = process.argv.slice(2), writeOutput = true) {
  ...
  if (args.help === "1" || args.h === "1") {
    const helpText = [
      ...
      "  --observer-wait-ms 5000 生成 sessionObserverToken 前的采集等待时间",
      "  --output json 同时输出 token / soToken / 长度等元数据（不会输出额外日志）",
    ].join("\n");
  ...
  const outputMode = String(pick(args.output, cfg("output"), process.env.SENTINEL_OUTPUT, "token")).toLowerCase();
  const observerWaitMSRaw = Number(pick(args["observer-wait-ms"], cfg("observerWaitMS", "observer_wait_ms"), process.env.SENTINEL_OBSERVER_WAIT_MS, 5000));
  const observerWaitMS = Number.isFinite(observerWaitMSRaw) && observerWaitMSRaw >= 0 ? Math.floor(observerWaitMSRaw) : 5000;
  const deviceId =
  ...
  const options = {
    ...
    async handleIframeMessage(message) {
      ...
      if (challengeFile) {
        if (!cachedChallenge) {
          const envelope = readChallengeEnvelope(challengeFile);
          cachedChallenge = envelope.cachedChallenge;
          cachedChallengeProof = envelope.cachedProof;
        }
      } else {
      ...
      return {
        cachedProof: cachedChallengeProof || proof,
  ...
  sdkCode = sdkCode
    .replace(
      "try{return await Nt(e.snapshot_dx)}catch{return null}",
      "try{return await Nt(e.snapshot_dx)}catch(t){globalThis.__sentinel_so_error=t?.message||String(t);return null}"
    )
    .replace(
      "function(t,n){const e=$(t??{})??\"\";return Tt((()=>Et(n,e)))}(t,e[n(8)])[n(25)]((()=>{}))",
      "function(t,n){const e=$(t??{})??\"\";return Tt((()=>Et(n,e)))}(t,e[n(8)])[n(25)]((t=>{globalThis.__sentinel_collector_error=t?.message||String(t)}))"
    );
  if (debugDx) {
  ...
  let soToken = "";
  let soAttempted = false;
  let soInitAttempted = false;
  let soError = "";
  const hasSessionObserver = typeof context.SentinelSDK.sessionObserverToken === "function";
  if (!hasSessionObserver) {
    soError = "sessionObserverToken unavailable";
  }
  if (hasSessionObserver && typeof context.SentinelSDK.init === "function") {
    soAttempted = true;
    soInitAttempted = true;
    try {
      await context.SentinelSDK.init(flow);
    } catch (error) {
      soError = error?.message || String(error);
    }
  }
  ...
  if (hasSessionObserver) {
    soAttempted = true;
    try {
      // Sentinel SDK may arm the SO collector either during init(flow) or only
      // after token(flow) receives the current req.so requirements.  Waiting
      // only before token(flow) misses the latter path and returns an empty
      // sessionObserverToken.  Match the browser flow by letting token(flow)
      // observe requirements first, then wait for the collector snapshot.
      if (observerWaitMS > 0) await sleep(observerWaitMS);
      const value = await context.SentinelSDK.sessionObserverToken(flow);
      soToken = typeof value === "string" ? value.trim() : value ? JSON.stringify(value) : "";
      if (!soToken && context.__sentinel_so_error) {
        soError = soError ? `${soError}; ${context.__sentinel_so_error}` : String(context.__sentinel_so_error);
      }
      if (!soToken && context.__sentinel_collector_error) {
        const collectorMessage = `collector: ${context.__sentinel_collector_error}`;
        soError = soError ? `${soError}; ${collectorMessage}` : collectorMessage;
      }
    } catch (error) {
      const message = error?.message || String(error);
      soError = soError ? `${soError}; ${message}` : message;
    }
  }
  clearTimers();
  if (!writeOutput) {
    return outputMode === "json"
      ? { token: tokenText, soToken, soAttempted, soInitAttempted, soError, observerWaitMS }
      : tokenText;
  }
  if (outputMode === "json") {
    const payload = {
      token: tokenText,
      soToken,
      sdkVersion: path.basename(sdkPath),
      tokenLen: tokenText.length,
      soTokenLen: soToken.length,
      soAttempted,
      soInitAttempted,
      soPresent: soToken.length > 0,
      observerWaitMS,
    };
    if (soError) payload.soError = soError;
