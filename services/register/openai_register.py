    ...
    def register(self, index: int) -> dict:
        ...
        try:
            ...
            if self.account_already_exists:
                # Outlook plus 别名常归一到主邮箱；主邮箱已有 OpenAI 账号时，本次随机密码无法登录。
                # 标记主邮箱已用并换下一个主邮箱重试，避免在 about-you 上死磕。
                try:
                    main = str((self.mailbox or {}).get("resolved_email") or email)
                    mail_provider.mark_outlook_external_main_used(main, reason="user_already_exists")
                    step(index, f"主邮箱已占用，已标记跳过: {main}", "yellow")
                except Exception:
                    pass
