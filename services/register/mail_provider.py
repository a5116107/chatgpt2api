...
def create_mailbox(mail_config: dict, username: str | None = None) -> dict:
    ...
    Previously only DDG daily-limit errors fell through, which made dual-route
    registration brittle when outlook_external was empty/exhausted.
    """
    enabled = _enabled_entries(mail_config)
    global provider_index
    with provider_lock:
        start = provider_index % len(enabled)
        provider_index = (provider_index + 1) % len(enabled)
    order = enabled[start:] + enabled[:start]

    tried: set[str] = set()
    errors: list[str] = []
    for entry in order:
        # Build provider from exact entry to preserve config order during failover.
        conf = _config(mail_config)
        ...
        try:
            ...
            return mailbox
        except Exception as error:  # noqa: BLE001 - provider-specific network/API errors
