...
class ProxyRuntimeProfile:
    ...
    profile_identity_key: str = ""
    sticky_session_key: str = ""
    user_agent: str = ""
...
class ProxySettingsStore:
    def __init__(
        ...
        self._clearance_cache: dict[tuple[str, str, str, str], ClearanceBundle] = {}
        self._provider_cache: dict[str, FlareSolverrClearanceProvider] = {}
        self._flight_locks: dict[tuple[str, str, str, str], threading.Lock] = {}
    ...
    def get_profile(
        ...
        runtime_profile_id, profile_identity_key, user_agent, impersonate = _profile_identity(account)
        ...
        if account_proxy:
            ...
            # Explicit per-call/per-feature proxy must win over the global runtime
            # proxy. Registration uses this path to keep the same configured
            # egress profile for mailbox + OpenAI requests.
            ...
            source = "global"
        sticky_session_key = _dynamic_proxy_session_key(runtime_profile_id, profile_identity_key)
        selected_proxy = _apply_dynamic_proxy_session(selected_proxy, sticky_session_key)
        ...
        return ProxyRuntimeProfile(
            ...
            sticky_session_key=sticky_session_key if normalize_proxy_url(selected_proxy) else "",
            user_agent=user_agent,
    ...
    def build_session_kwargs(
        ...
        **session_kwargs,
        ...
        session_kwargs.setdefault("impersonate", profile.impersonate or "chrome146")
    ...
    def get_runtime_status(self) -> dict[str, object]:
        ...
        with self._lock:
            cached_hosts = [host for _profile_id, _proxy, host, _identity in self._clearance_cache]
            cached_profiles = [profile_id for profile_id, _proxy, _host, _identity in self._clearance_cache if profile_id]
        ...
        return {
            ...
            "sticky_session_enabled": bool(profile.sticky_session_key),
            "sticky_session_key_preview": profile.sticky_session_key[:12] if profile.sticky_session_key else "",
            ...
            "cached_clearance_count": cached_count,
            "cached_clearance_hosts": sorted(set(cached_hosts)),
            "cached_clearance_profiles": sorted(set(cached_profiles)),
        }
    ...
    def _get_flight_lock(self, key: tuple[str, str, str, str]) -> threading.Lock:
        with self._lock:
    ...
    def _get_cached_bundle(self, key: tuple[str, str, str, str]) -> ClearanceBundle | None:
        with self._lock:
            return self._clearance_cache.get(key)

    def _set_cached_bundle(self, key: tuple[str, str, str, str], bundle: ClearanceBundle) -> None:
        with self._lock:
            self._clearance_cache[key] = bundle

    @staticmethod
    def _cache_key(profile: ProxyRuntimeProfile, target_host: str) -> tuple[str, str, str, str]:
        return (
            _clean(profile.runtime_profile_id),
            normalize_proxy_url(profile.proxy_url),
            _normalize_host(target_host),
            _clean(profile.profile_identity_key),
        )
...
def _profile_identity(account: dict | None) -> tuple[str, str, str, str]:
    source = account if isinstance(account, dict) else {}
    snapshot = source.get("profile_snapshot") if isinstance(source.get("profile_snapshot"), dict) else {}
    fp = source.get("fp") if isinstance(source.get("fp"), dict) else {}
    headers = snapshot.get("headers") if isinstance(snapshot.get("headers"), dict) else {}
    tls = snapshot.get("tls") if isinstance(snapshot.get("tls"), dict) else {}
    runtime_profile_id = (
        _clean(source.get("runtime_profile_id"))
        or _clean(snapshot.get("id"))
    )
    user_agent = _clean(fp.get("user-agent")) or _clean(headers.get("user-agent"))
    impersonate = _clean(fp.get("impersonate")) or _clean(tls.get("impersonate")) or "chrome146"
    identity_parts = {
        "runtime_profile_id": runtime_profile_id,
        "user-agent": user_agent,
        "impersonate": impersonate,
        "sec-ch-ua": _clean(fp.get("sec-ch-ua")) or _clean(headers.get("sec-ch-ua")),
        "sec-ch-ua-platform": _clean(fp.get("sec-ch-ua-platform")) or _clean(headers.get("sec-ch-ua-platform")),
        "accept-language": _clean(fp.get("accept-language")) or _clean(headers.get("accept-language")),
    }
    identity_raw = json.dumps(identity_parts, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    identity_key = hashlib.sha256(identity_raw.encode("utf-8")).hexdigest()[:24]
    return runtime_profile_id, identity_key, user_agent, impersonate


def _is_dynamic_proxy_endpoint(url: str) -> bool:
    parsed = urlparse(normalize_proxy_url(url))
    host = _normalize_host(parsed.hostname or "")
    port = parsed.port
    return bool(host) and ("dynamic-proxy" in host or (host in {"127.0.0.1", "localhost"} and port in _DYNAMIC_PROXY_PORTS) or port in _DYNAMIC_PROXY_PORTS)


def _dynamic_proxy_session_key(runtime_profile_id: str, profile_identity_key: str) -> str:
    seed = _clean(runtime_profile_id) or _clean(profile_identity_key)
    if not seed:
        return ""
...
def _apply_dynamic_proxy_session(url: str, session_key: str) -> str:
    ...
    if not host:
        return candidate
    netloc = f"{quote(session_key, safe='')}:{quote('x', safe='')}@{host}"
    if parsed.port:
...
def test_proxy(url: str = "", *, timeout: float = 15.0) -> dict:
    ...
    session = Session(**proxy_settings.build_session_kwargs(proxy=candidate, upstream=True, impersonate="edge101"))
