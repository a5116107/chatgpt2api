...
export function RegisterCard() {
  ...
  const handleToggleRegister = () => {
    if (!config) return;
    if (!config.enabled) {
      const ok = window.confirm(`确认启动注册任务？当前配置：${config.threads} 线程，模式 ${config.mode || "total"}，请确认代理和邮箱配置已经保存。`);
      if (!ok) return;
    }
    void toggle();
  };
    ...
    <div className="grid h-[calc(100vh-132px)] min-h-[640px] items-stretch gap-0 overflow-hidden rounded-xl border border-stone-200 bg-white/70 xl:grid-cols-2">
      <section className="space-y-4 overflow-y-auto border-b border-stone-200 p-4 xl:border-r xl:border-b-0">
          ...
          <div className="flex items-start gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs leading-5 text-sky-800">
            ...
            <span>如果注册日志出现 Cloudflare 拦截，可在设置页启用 FlareSolverr 清障；使用自建 mailfree 时，留空的 Admin Token/Password 会自动回退到后端容器环境变量 `MAIL_ADMIN_TOKEN` / `MAILFREE_ADMIN_TOKEN`。</span>
          </div>
          ...
          <div className="space-y-3 border-t border-stone-200 pt-3">
            ...
            <div className="space-y-3">
              {providers.map((provider, index) => {
                const type = String(provider.type || "mailfree");
                const domains = Array.isArray(provider.domain) ? provider.domain.map(String).join("\n") : "";
                const domainIndices = Array.isArray(provider.domain_indices) ? provider.domain_indices.map(String).join(",") : String(provider.domain_index || "");
                ...
                return (
                  <div key={index} className="space-y-3 border-t border-stone-200 pt-3 first:border-t-0 first:pt-0">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      {type === "cloudmail_gen" || type === "cloudflare_temp_email" || type === "mailfree" || type === "moemail" || type === "inbucket" || type === "yyds_mail" || type === "ddg_mail" ? (
                        <>
                          <div className="space-y-2">
                          ...
                          {type === "cloudflare_temp_email" || type === "mailfree" || type === "ddg_mail" ? (
                            <div className="space-y-2">
                              <label className="text-sm text-stone-700">{type === "mailfree" ? "Admin Token / Password" : "Admin Password"}</label>
                              <Input value={String(provider.admin_password || "")} onChange={(event) => updateProvider(index, { admin_password: event.target.value })} placeholder={type === "mailfree" ? "留空则使用容器环境变量 MAIL_ADMIN_TOKEN / MAILFREE_ADMIN_TOKEN" : ""} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
                            </div>
                          ) : null}
                        </>
                      ) : null}
                      ...
                      {type === "mailfree" ? (
                        <div className="space-y-2">
                          <label className="text-sm text-stone-700">Domain Index / Indices</label>
                          <Input
                            value={domainIndices}
                            onChange={(event) => updateProvider(index, { domain_indices: event.target.value.split(/[\n,]/).map((item) => Number(item.trim())).filter((item) => Number.isInteger(item) && item >= 0) })}
                            placeholder="4 或 482,1383,1443,..."
      ...
      <section className="flex min-h-0 flex-col p-4">
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Button className="h-10 rounded-xl bg-stone-950 px-3 text-white hover:bg-stone-800" onClick={handleToggleRegister} disabled={isSaving}>
            ...
            <div className="flex items-center gap-2 border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              启动之前必须先保存配置；启动请求会经过二次确认，旧页面或脚本的空请求不会拉起注册任务。
            </div>
        </div>
